    ...
    uint32_t ret_val)
{
    ...
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 start*/
    memcpy(&g_rslt_data[g_rslt_data_len], testdata, len);
    g_rslt_data_len += len;
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 end*/
#else
    memcpy(&g_rslt_data[g_rslt_data_len], testdata, len);
    g_rslt_data_len += len;
#endif
...
static void hx_print_fw_info(void)
{
    ...
    int panel_ver;
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 start*/
    uint8_t cus_info[12] = {0};
    uint8_t proj_info[16] = {0};
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 end*/
    uint8_t data[12] = {0};
    uint8_t tmp_addr[4];
    int buf_size = 1024;
...
static char *get_date_time_str(void)
{
    ...
    snprintf(time_data_buf, sizeof(time_data_buf), "%04d%02d%02d_%02d%02d%02d_",
...
static int himax_self_test_data_init(void)
{
    ...
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 start*/
    char *file_name_2 = "hx83112f_txd_boe_mp_test.csv";
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 end*/
    int setting_sz = -1;
    int ret = HX_INSP_OK;
    int err = 0;
...
static int himax_chip_self_test_for_tp(void)
{
    ...
    if (ret > 0) {
        E("%s: initialize self test failed\n", __func__);
        if (ret == HX_INSP_EFILE) {
           E("Self_Test Fail: Criteria file error!\n");
        }
    ...
    if (private_ts->debug_log_level & BIT(4)) {
        ...
        g_core_fp.fp_power_on_init();
        g_core_fp.fp_read_FW_ver();
        g_mp_fw_ver = (ic_data->vendor_cid_maj_ver << 8 | ic_data->vendor_cid_min_ver);
    }
#endif
    hx_print_criteria_ver();
...
SELF_TEST_FAIL:
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 start*/
    if (test_result) {
        sprintf(g_file_name, "%s", "Fail");
    } else {
        sprintf(g_file_name, "%s", "Pass");
    }
    snprintf(g_file_path, (int)(strlen(HX_RSLT_OUT_PATH)
            + strlen(g_file_name) + strlen(get_date_time_str()) + strlen(HX_RSLT_OUT_FILE) + 6),
            "%s%s_%s%s", HX_RSLT_OUT_PATH, g_file_name, get_date_time_str(), HX_RSLT_OUT_FILE);
    file_w_flag = true;
...
#if defined(HX_RW_FILE)
    if (file_w_flag) {
        vfs_write(raw_file, g_rslt_data, g_rslt_data_len* sizeof(char), &pos);
        pos += g_rslt_data_len;
    }
    /*hs14 code for AL6528A-534 by zhangxiongyi at 20221101 end*/
...
UPDATE_MPFW_FAIL:
    ...
    for (j = 0; j < HX_CRITERIA_ITEM - 1; j++) {
        if (g_test_item_flag[j] == 1) {
            if (j <= i) {
                I("%s : %s\n", g_himax_inspection_mode[j], (rslt & (1 << j)) ? "FAIL" : "PASS");
            } else {
                I("%s : %s\n", g_himax_inspection_mode[j], "SKIP");
            }
        }
...
void himax_inspection_init(void)
{
    ...
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 start*/
    g_core_fp.fp_chip_self_test_for_tp = himax_chip_self_test_for_tp;
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 end*/
}
