...
static int himax_mcu_chip_self_test_for_tp(void)
{
    uint8_t tmp_data[FLASH_WRITE_BURST_SZ];
    uint8_t self_test_info[20];
    int pf_value = 0x00;
    ...
    g_core_fp.fp_interface_on();
    g_core_fp.fp_sense_off(true);
    g_core_fp.fp_register_write(pfw_op->addr_selftest_addr_en, pfw_op->data_selftest_request, DATA_LEN_4);
    /*Set criteria 0x10007F1C [0,1]=aa/up,down=, [2-3]=key/up,down,
     *        [4-5]=avg/up,down
     */
    tmp_data[0] = pfw_op->data_criteria_aa_top[0];
    ...
    tmp_data[6] = 0x00;
    tmp_data[7] = 0x00;
    g_core_fp.fp_register_write(pfw_op->addr_criteria_addr, tmp_data, FLASH_WRITE_BURST_SZ);
    g_core_fp.fp_register_write(pfw_op->addr_set_frame_addr, pfw_op->data_set_frame, DATA_LEN_4);
    /*Disable IDLE Mode*/
    g_core_fp.fp_idle_mode(1);
    /*Diable Flash Reload*/
    g_core_fp.fp_reload_disable(1);
    ...
    /*Hand shaking*/
    for (i = 0; i < 1000; i++) {
        g_core_fp.fp_register_read(pfw_op->addr_selftest_addr_en, tmp_data, 4);
        I("%s:data0=0x%2X,data1=0x%2X,data2=0x%2X,data3=0x%2X cnt=%d\n",
            __func__,
            tmp_data[0],
            tmp_data[1],
     ...
     * Read test result ==> bit[2][1][0] = [key][AA][avg] => 0xF = PASS
     */
    g_core_fp.fp_register_read(pfw_op->addr_selftest_result_addr, self_test_info, 20);
    test_result_id = self_test_info[0];
    I("%s: check test result, test_result_id=%x, test_result=%x\n", __func__, test_result_id, self_test_info[0]);
    I("raw top 1 = %d\n", self_test_info[3] * 256 + self_test_info[2]);
    I("raw top 2 = %d\n", self_test_info[5] * 256 + self_test_info[4]);
...
/* CORE_FW */
    ...
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 start*/
    g_core_fp.fp_chip_self_test_for_tp = himax_mcu_chip_self_test_for_tp;
    /*hs14 code for SR-AL6528A-01-512 by hehaoran5 at 20220911 end*/
