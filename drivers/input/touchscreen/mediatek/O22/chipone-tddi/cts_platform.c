               ...
               const void *src, size_t len, int retry, int delay)
{
    ...
    if (pdata->cts_dev->rtdata.program_mode) {
        if ((!pdata->cts_dev->hwdata) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911C) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916C) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9922))
        {
        ...
        if ((!pdata->cts_dev->hwdata) ||
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9922C))
        {
            data_len = len - 3;
            pdata->spi_tx_buf[0] = dev_addr;
            ...
            do {
                ret = cts_spi_send_recv(pdata, len + 15, pdata->spi_tx_buf, pdata->spi_rx_buf);
                if (ret) {
                    cts_err("SPI write failed %d", ret);
                    if (delay)
                        mdelay(delay);
              ...
              int retry, int delay)
{
    ...
    if (pdata->cts_dev->rtdata.program_mode) {
            ...
            (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9922C))
        {
            ...
            do {
                ret = cts_spi_send_recv(pdata, wlen + rlen + 15, pdata->spi_tx_buf, pdata->spi_rx_buf);
                if (ret) {
                 ...
                 size_t rlen, int retry, int delay, int idle)
{
    ...
    if (pdata->cts_dev->rtdata.program_mode) {
        if (pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916 ||
            pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916C ||
            pdata->cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9922) {
            pdata->spi_tx_buf[0] = dev_addr | 0x01;
            memcpy(&pdata->spi_tx_buf[1], wbuf, wlen);
...
static int cts_input_open(struct input_dev *dev)
{
    tp_cts_data->pdata->tp_is_enabled = 1;
    return 0;
}

static void cts_input_close(struct input_dev *dev)
{
    tp_cts_data->pdata->tp_is_enabled = 0;
}
...
#endif
{
    ...
    /*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 20220926 start*/
    input_dev->open = cts_input_open;
    input_dev->close = cts_input_close;
    /*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 20220926 end*/
