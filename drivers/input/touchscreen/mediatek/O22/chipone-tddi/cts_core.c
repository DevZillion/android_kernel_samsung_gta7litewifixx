...
static unsigned int prox_cur_status = 2;    //1: near; 2: far; default 2;
...
void cts_set_prox_en(unsigned int enable);
                     ...
                     u32 crc, int retry)
{
    ...
    do {
        ...
        ret =
            cts_sram_writesb_boot_crc_retry(cts_dev, len,
                         fw_crc, 3);
        if (ret != 0) {
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if (cts_dev->pdata->tp_is_enabled) {
